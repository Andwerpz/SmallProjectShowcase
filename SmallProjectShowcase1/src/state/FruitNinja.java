...
//		is.doGravity = false;
		...
		// ground
		// Body b = is.add(new Polygon(MainPanel.WIDTH / 2, 5.0f), MainPanel.WIDTH / 2,
		// 0);
		// b.setStatic();
		// b.setOrient(0);
	}
	...
	public void tick(Point mouse2) {
		...
		if(counter % 180 == 0) {
			...
			Body b = is.add(new Polygon(verts), MainPanel.WIDTH / 2, -100);
			b.setOrient(ImpulseMath.random(-ImpulseMath.PI, ImpulseMath.PI));
			b.velocity.set(Math.random() * 120 - 60, 250);
			b.restitution = 0.2f;
			...
			for (int i = is.bodies.size() - 1; i >= 0; i--) {
				...
				if(b.shape instanceof Polygon) {
					...
					for (int j = 0; j < p.vertexCount; j++) {
						...
						Vec2 intersect = MathTools.line_lineCollision(v1.x, v1.y, v2.x, v2.y, modelPressed.x,
								modelPressed.y, modelReleased.x, modelReleased.y);
						if(intersect != null) {
							if(i1 == null) {
								// first intersect
								i1 = intersect;
					...
					// add random rot to cut polys
					b1.angularVelocity = b.angularVelocity + Math.random() * Math.toRadians(30) - Math.toRadians(15);
					b2.angularVelocity = b.angularVelocity + Math.random() * Math.toRadians(30) - Math.toRadians(15);
		...
		// purge polys below screen
		for (int i = is.bodies.size() - 1; i >= 0; i--) {
			if(is.bodies.get(i).position.y < -200) {
				is.bodies.remove(i);
			}
		}
	}
	...
	public void draw(Graphics g) {
		...
		if(mousePressed) {
			// draw cutting line
			g.drawLine((int) vecPressed.x, (int) (-(vecPressed.y - MainPanel.HEIGHT / 2) + MainPanel.HEIGHT / 2),
