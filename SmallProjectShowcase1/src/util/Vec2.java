...
public class Vec2 {
	...
	public double length() {
		return (double) StrictMath.sqrt(x * x + y * y);
	}
	...
	public void normalize() {
		...
		if (lenSq > 0.000000001) {
			double invLen = 1.0f / (double) StrictMath.sqrt(lenSq);
			x *= invLen;
			y *= invLen;
		}
	}
	...
	public static Vec2 min(Vec2 a, Vec2 b, Vec2 out) {
		out.x = (double) StrictMath.min(a.x, b.x);
		out.y = (double) StrictMath.min(a.y, b.y);
		return out;
	}

	public static Vec2 max(Vec2 a, Vec2 b, Vec2 out) {
		out.x = (double) StrictMath.max(a.x, b.x);
		out.y = (double) StrictMath.max(a.y, b.y);
		return out;
	}
	...
	public static double distance(Vec2 a, Vec2 b) {
		...
		return (double) StrictMath.sqrt(dx * dx + dy * dy);
	}
